...
from .service import ClubMembershipService
...
def list_by_club(club_id: str, status: str | None):
    try:
        items = service.list_by_club(club_id, status=status)
        return jsonify(schema.dump(items, many=True)), 200
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:
...
def approve(record_id: str):
    try:
        item = service.approve_membership(record_id)
        return jsonify(schema.dump(item)), 200
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:
...
def reject(record_id: str):
    try:
        item = service.reject_membership(record_id)
        return jsonify(schema.dump(item)), 200
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:
